...
/* 提前收局展示时长：双下/三下(头游整队先出完)或只剩末游一人时，胜负已定，没必要逼剩下的
   人继续打完——直接把他们手里剩的牌摊开给全场看 10 秒，再进结算/重新发牌。 */
const LAST_PLAY_DISPLAY_MS = 10000;
...
async function applyTakeover(io, state, seat) {
  ...
  if (state.tributePhase) {
    await driveTributeBots(io, state);          // 供牌/还供阶段：机器人自动接替
  } else if (state.turnSeat === seat && state.turnTimer) {
    startTurnTimer(io, state);                  // 正轮到 → 缩短为托管计时
  }
...
async function _writeRoundResult(io, state, result, is6p, new1 = 0, new2 = 0, tributeJson = null) {
  ...
  /* 原来是每个玩家一条顺序 UPDATE（四人房最多4次往返），改成两条批量 UPDATE——
...
async function startTributePhase(io, roomCode, tributeInfo) {
  ...
  /* 首出座位：
     a) 进贡最大的那张牌的贡牌方先出——exchanges 继承自 giversByTop 按贡牌大小降序排列，
        exchanges[0] 就是贡牌最大的人，不分双下还是单人进贡都适用；
     b) 双下且并列最大(比如都进贡级牌，大小相同、分不出谁更大)时，才退回"头游的下家"
        这条平手仲裁规则。旧版双下时无条件用"头游的下家"，完全没看谁贡的牌大——
        tributeLeadId 当时其实已经算出"贡给头游的那个人"却从未真正用来定 leadSeat，
        是遗留的半成品，这次一并接上。 */
...
async function finalizeTribute(io, state) {
  ...
  await persistState(state);
  await query(`UPDATE gdo_rooms SET tribute_json=NULL WHERE room_code=$1`, [state.roomCode]);
...
async function applyPlay(io, state, playerId, cards, isAuto = false) {
  ...
  if (headDone >= teamSize || state.finishOrder.length >= state.totalPlayers - 1) {
    /* 剩余玩家(负方)按当前顺序补入名次；他们手里可能还有一整把没打完的牌——胜负已经
       没有悬念(双下/三下已提前锁定，或就剩这一人注定垫底)，摊牌快照要在写入名次前取，
    ...
    /* 先把最后一手牌广播出去，让所有玩家看清楚最后出的是什么牌型；
       顺带清掉回合倒计时，避免展示期间还有一个走完的倒计时环在转——
       此时胜负已定、没人需要也不能再出牌，不存在"轮到谁却卡住"的死锁风险。*/
    clearTurnTimer(state);
...
/* 供 matchmaking 在发牌开局时调用：把开局即离线(未回座)的座位直接置为机器人托管 */
module.exports.seedDisconnectedFromDb = function(io, state, seatConnMap) {
  ...
  /* 若开局即处于进贡阶段，托管座位由机器人自动供/还，避免卡死 */
  if (state.tributePhase) driveTributeBots(io, state).catch(e => console.error('[seed-tribute]', e.message));
};
