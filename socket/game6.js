...
async function persistState(state) {
  await query(
    `UPDATE gdo6_rounds SET current_hands_json=$1, turn_state_json=$2 WHERE id=$3`,
    [
...
async function rebuildState(roomCode, seat, round) {
  const allSeats = await query(`
    ...
    FROM gdo6_seats s JOIN gdo_players p ON p.id = s.player_id
...
async function finishRound(io, state) {
  ...
  const room = await queryOne('SELECT a_fails_team1, a_fails_team2 FROM gdo6_rooms WHERE room_code=$1', [state.roomCode]);
...
async function finishRound6p(io, state) {
  ...
  const room = await queryOne('SELECT a_fails_team1, a_fails_team2, room_type FROM gdo6_rooms WHERE room_code=$1', [state.roomCode]);
  const adj = applyAWinRule(state, result, parseInt(room.a_fails_team1 || 0), parseInt(room.a_fails_team2 || 0));
  result.newLv1 = adj.newLv1; result.newLv2 = adj.newLv2; result.guoA = adj.guoA;

  /* 进贡信息（写入下局使用；过A重开则不进贡）。
     控制台「是否进贡」设为 NO 时：不供不还，改由本局头游先出（buildTributeJson 内处理）。*/
  const tributeJson = await buildTributeJson(state, result, adj, '6p', room.room_type, computeTribute6p);
...
async function startTributePhase(io, roomCode, tributeInfo) {
  ...
  const teamSize    = Math.floor(state.totalPlayers / 2);   // 6人=3

  /* 抗贡（六人）：进贡者手中「合计」大王 ≥ teamSize(3) 即整方抗贡 */
  let totalBJ = 0;
  ...
  while (progressed && state.tributePhase && guard++ < 50) {
    ...
    if (tr && tr.remaining.length) {
      ...
      if (isPlayerSeatDisconnected(state, activeGiverId)) {
        ...
        } catch (e) { console.error('[driveTributeBots6:tie]', e.message); }
      }
    }
  }
}
...
async function closeAbandonedRoom(io, state) {
  try {
    ...
    console.log('[房间6] 🚪 全部托管(无人在线)，立即关闭 · ' + state.roomCode);
  } catch (e) { console.error('[closeAbandonedRoom6]', e.message); }
}
  ...
  socket.on('game:request_hand', async function(data) {
    try {
      ...
      const round = await queryOne(`
        SELECT * FROM gdo6_rounds WHERE room_id=$1 ORDER BY round_number DESC LIMIT 1
      `, [seat.room_id]);
      ...
      if (!round || !round.hands_json) {
        ...
        const roomInfo = await queryOne(
          'SELECT room_code, room_type, level_team1, level_team2 FROM gdo6_rooms WHERE id=$1',
          [seat.room_id]
        );
        socket.emit('game:waiting', {
          roomCode, mode: '6p', roomType: roomInfo.room_type,
        ...
        io.to(roomCode).emit('game:seat_update', {
          seats: allSeats, roomCode, mode: '6p', roomType: roomInfo.room_type
        });
        return;
      }
