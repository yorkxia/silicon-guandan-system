...
module.exports = function(io, socket) {
  ...
  socket.on('queue:join', async function(data) {
    try {
      ...
      const revival = await findRevivalRoom();
      if (revival) {
        ...
        if (!rr.error) {
          ...
          if (rst.seats.length >= 6) await dealAndStart(io, revival, rst);
          return;
        }
  ...
  socket.on('room:leave', async function(data) {
    try {
      const { roomCode } = data;
      await query(`UPDATE gdo6_seats SET is_connected=FALSE WHERE socket_id=$1`, [socket.id]);
