...
const { initGameState, startTributePhase, seedDisconnectedFromDb, remapSeatOwner } = require('./game');
...
module.exports = function(io, socket) {
  ...
  socket.on('room:create', async function(data) {
    try {
      ...
      /* 房间刚建出必然0座位，直接插第一个座位，不必再走 joinRoomByCode/getRoomState
         各自的查重+查满员+重新查询(省4次DB往返)——建房瞬间高并发时这几次省下来的
         连接池占用很关键，见压测结论(2026-09-19)。 */
